...
filepath <- "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\IMR575_HistoneAnalysis\\EpiPro2.2.csv"
...
# Sample ID import
filepath <- "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\IMR575_HistoneAnalysis\\IMR575_samples_modified.csv" 
sample_ids <- read.csv(filepath)
# clean the sample_ids dataset
mask <- !colSums(is.na(sample_ids)) == nrow(sample_ids)
...
mappings <- setNames(sample_ids$clean_name, sample_ids$Sample.ID)
...
bad_samples <- c(
    "DOX--B2-T1" # <- Stephanie removed this one too, it has too many missing values 
   ,"DOX--B1-T2"  # <- I want to remove this as it 
   ,"DOX--B1-T1"
)
...
groups <- merge(histone_data, 
                select(sample_ids,c(clean_name,Group.name)),
                by.x = "Sample",
                by.y ="clean_name")
group_flags <- groups %>%
  mutate(across(
...
write_csv(histone_data,"C:\\Users\\Tim\\OneDrive\\__ThesisProject\\Results\\Appendix1_PoorSamples\\IMR_filtered_ids.csv")
...
# Write normalized csv file from here...
write.csv(normalized, "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\Raw_histone_data\\imr575norm.csv")
...
groups <- rename(groups, Sample_Type = Group.name )
...
groups <- merge(normalized, select(sample_ids, c(Group.name, Sample)), by ="Sample")
groups <- rename(groups,
                 Sample_Type = Group.name)

write.csv(groups,
          "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\Histone_be2IMR_comparative\\IMRgroups.csv"
)
...
write.csv(average,
          "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\Histone_be2IMR_comparative\\IMRgroups_average.csv"
)
...
ctrl_dpi <- groups[groups$Sample_Type %in% c("DOX-","DOX-DPI+"),]
...
write_csv(long_stat_tim, "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\IMR575_HistoneAnalysis\\IMR575_t-testvscontrol_students.csv")
...
write.csv(final, file = "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\IMR575_HistoneAnalysis\\output_df_2.csv")
...
write.csv(deconvoluted,
          "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\Histone_be2IMR_comparative\\IMRgroups_deconvol.csv"
)
...
write.csv(average_deconv,
          "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\Histone_be2IMR_comparative\\IMRgroups_deconvol_avg.csv"
)
...
# ============================= Student t-tests with FDR for control vs DPI 
ctrl_dpi <- deconvoluted[deconvoluted$Sample_Type %in% c("DOX-","DOX-DPI+"),]
...
write_csv(long_stat_tim_deconv, "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\IMR575_HistoneAnalysis\\IMR575_t-testvscontrol_students_deconv.csv")
...
write.csv(final_deconv, file = "C:\\Users\\Tim\\OneDrive\\__ThesisProject\\_DryLab\\Code\\IMR575_HistoneAnalysis\\IMR575Deconv_results.csv")
